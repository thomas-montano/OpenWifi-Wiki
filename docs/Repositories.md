...
openwifi is a full-stack, Linux `mac80211`-compatible IEEE 802.11 (Wi-Fi) implementation for SDR hardware. The **radio PHY and real-time MAC live in FPGA fabric**. The **driver and everything above it are ordinary Linux**. The FPGA uses Verilog and Vivado, while the driver uses C and the kernel build system. The two halves live in **separate repositories**.

## Why more than one repository?

The project has four repositories, each with its own job and toolchain.
...
<figcaption><em>The four openwifi repositories and how they connect. If you develop on openwifi, the two you clone day-to-day are <strong>openwifi</strong> and <strong>openwifi-hw</strong>. The other two supply the OFDM receiver and prebuilt FPGA images.</em></figcaption>
</figure>

| Repository | What it holds | When you touch it |
|---|---|---|
| **[openwifi](https://github.com/open-sdr/openwifi)** | The Linux kernel driver, `sdrctl` and other user-space tools, capture and demo scripts, SD-card boot files (kernel patches, device trees, U-Boot), and **all the documentation** | Rebuilding the driver, changing runtime behavior, writing scripts, flashing and booting a board, reading app notes |
...
| **[openwifi-hw-img](https://github.com/open-sdr/openwifi-hw-img)** | **Prebuilt FPGA bitstreams** for each board (`.xsa`, bitstream, ILA `.ltx`, init files under `boards/<board_name>/sdk/`), plus the CLA and documentation PDFs | You want a working bitstream *without* installing Vivado or waiting hours for synthesis |
...
!!! tip "One `board_name` across all repositories"
    The same `board_name` string (for example `zed_fmcs2`, `zcu102_fmcs2`) names a board in all the repos. It selects `openwifi-hw/boards/$BOARD_NAME/` (FPGA project), `openwifi-hw-img/boards/$BOARD_NAME/sdk/` (prebuilt bitstream), and `openwifi/kernel_boot/boards/$BOARD_NAME/` (boot files and device tree). Set `export BOARD_NAME=...` once and every build script uses it. See [Supported Boards](Supported-Boards.md).

## How the pieces fit at build and run time

- **At run time**, three things run on the board. These are the FPGA bitstream (`system_top.bit.bin`), the kernel driver (`sdr.ko` and related modules), and the user-space tools. The driver and tools come from `openwifi`. The bitstream is built from `openwifi-hw` or downloaded from `openwifi-hw-img`.
- **At build time**, the driver must agree with the FPGA on the **register map**. `openwifi/driver/hw_def.h` holds the addresses the driver writes, while each core's `*_s_axi.v` in `openwifi-hw/ip/` implements the registers in the FPGA. A driver file and its FPGA counterpart usually share a name (`xpu.c` and `xpu.v`). The two repos are versioned together because those definitions must match.
- **The submodule chain**: `openwifi-hw` pulls in two submodules. The Analog Devices FPGA reference design, [`analogdevicesinc/hdl`](https://github.com/analogdevicesinc/hdl), sits at `adi-hdl/` and is pinned to tag `2022_R2`. openwifi is built on top of it. `openofdm` (branch `dot11zynq`) sits at `ip/openofdm_rx/`. A fresh `openwifi-hw` clone has these as **empty directories** until you run `./prepare_adi_lib.sh` and `./get_ip_openofdm_rx.sh` (or `git submodule update --init`).

## Versions this wiki targets

openwifi pins several toolchains and upstream projects, and they intentionally don't all share a version number. A 2022-era FPGA toolchain paired with a current kernel is normal here. If a build script or a repo README states a version different from the table below, trust the repo. Treat this table as a snapshot from the last reconciliation with the repos (**September 26, 2026**). Bumping it when they move is a welcome contribution.

| Component | Target version | Set or pinned in | Notes |
|---|---|---|---|
| Vivado + Vitis | **2022.2** (needs `Vitis`, not `Vitis_HLS`) | openwifi-hw build scripts | Free tier suffices for Zynq-7020 boards. ZC706, ZCU102, and Z7035 need a license to *rebuild* the FPGA. See [FPGA Development](FPGA-Development.md#prerequisites). |
...
| ADI Linux kernel (`adi-linux` and `adi-linux-64`) | branch **`2026_R1`**, Linux **6.12** | `prepare_kernel.sh` | Patched by `ad9361_v6_12.patch`. See [Boot, Kernel & Device Tree](Boot-Kernel-Device-Tree.md#the-kernel). |
...
| Build-host OS | **Ubuntu 18, 20, or 22 LTS** | not pinned | Ubuntu 24 needs `libtinfo5` installed manually (see [Environment Setup](Development-Environment-Setup.md#host-os-and-packages)). |
...
### …the driver and how Linux talks to the hardware
...
### …the PHY (OFDM modulation and demodulation) or the real-time MAC (CSMA/CA, ACK, TSF)
`openwifi-hw/ip/`: one directory per IP core. `xpu/` is the real-time MAC, and `openofdm_tx/` and `openofdm_rx/` are the PHY. `tx_intf/` and `rx_intf/` are the DAC-side and ADC-side interfaces, and `side_ch/` handles CSI and IQ capture. See [FPGA IP Cores](FPGA-IP-Cores.md).

### …CSI and IQ capture (the research features)
Three places cooperate: `openwifi-hw/ip/side_ch/` (the FPGA capture engine), `openwifi/driver/side_ch/` (the `side_ch.ko` kernel module), and `openwifi/user_space/side_ch_ctl_src/` (the `side_ch_ctl` tool plus the Python and MATLAB display scripts). See [Research Features](Research-Features.md).

### …how a board boots (kernel, device tree, U-Boot, BOOT.BIN)
`openwifi/kernel_boot/`: per-board boot artifacts under `boards/<board_name>/`, kernel patches, and the device-tree overlay machinery (`construct_device_tree.sh`, `openwifi_32_ad9361.dtso`, and `openwifi_64_ad9361.dtso`). See [Boot, Kernel & Device Tree](Boot-Kernel-Device-Tree.md).

### …adding or porting a board
Both repos: `openwifi-hw/boards/<board_name>/` (Vivado project) and `openwifi/kernel_boot/boards/<board_name>/` (device tree and boot files). See [FPGA Development → Porting to a new board](FPGA-Development.md#porting-to-a-new-board).

### …a prebuilt bitstream (skip synthesis)
[`openwifi-hw-img`](https://github.com/open-sdr/openwifi-hw-img), `boards/<board_name>/sdk/`.

### …the authoritative docs and app notes
...
All openwifi repositories are **dual-licensed**. [AGPLv3](https://www.gnu.org/licenses/agpl-3.0.en.html) covers the open-source release, and [openwifi.tech](https://openwifi.tech) offers commercial licenses for advanced features. Individual files may be GPL-2.0-or-later or BSD-3-Clause. Vendored third-party components (Analog Devices HDL, the Xilinx Viterbi decoder, openofdm) carry their own terms, so check per-file headers for your use case.
...
The project originated at **Ghent University and imec** (Xianjun Jiao, Wei Liu, Michael Mehari, and contributors). It was funded by the EU H2020 [ORCA project](https://www.orca-project.eu/) (grant 732174) and by [NLnet](https://nlnet.nl/) through NGI Zero. See [FAQ & Resources](FAQ-and-Resources.md) for citation info and the publication list.
